...
> - **빠른 분석 모드 (Lite)**: 토큰 소비를 줄이려면 하네스 초기화 시 `"빠르게 하네스 구축해줘"` 혹은 `"skip spec"` 문구를 포함해 실행하세요.
...
| `skills/sync-wiki/SKILL.md` | 시스템 wiki를 폴더(wiki/)와 MSSQL DB(harness_wiki_pages) 사이에서 상호 변환 |
...
| 시스템 wiki 저장 위치(폴더/DB) 상호 변환 | `sync-wiki` |
