...
> **주의**: 이 질문은 옵션이 5개다. `AskUserQuestion` 툴은 옵션 최대 4개까지만 지원해 5번(허브형 1:N)이 잘려나간다. 반드시 평문 텍스트로 아래 형식 그대로 출력하고 사용자의 자유 텍스트 응답(1/2/3/4/5)을 받는다 — `AskUserQuestion` 툴을 쓰지 않는다.
