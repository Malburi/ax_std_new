...
| 파일 | 필수 요소 |
|------|---------|
| `gemini.md` | 프로젝트명, 요청 흐름, 빌드/실행, 자동 워크플로우 테이블, 변경 이력 (500줄 초과 시 경고) |
| `.gemini/skills/trace.md` | frontmatter (name/description/model), 단계별 탐색 절차, 출력 형식 |
| `.gemini/skills/scaffolder.md` | frontmatter, 파일 생성 체크리스트, 실제 경로 패턴 |
| `.gemini/skills/find-logic.md` | frontmatter, 역방향 탐색 절차 |
| `.gemini/agents/domain-expert.md` | frontmatter, 스택 정보, 코드 컨벤션 |
| `.gemini/patterns/` | 최소 1개 파일 존재, 각 300줄 이하 |
...
gemini.md의 자동 워크플로우 테이블에 모든 스킬이 등록되었는지 확인.
...
`.gemini/skills/harness-init.md`가 writer에 의해 삭제·덮어쓰기되지 않았는지 확인.

### 9. NEW — 변경 이력 기록 확인

gemini.md의 "변경 이력" 테이블에 이번 실행의 항목이 추가되었는지 확인. 누락 시 WARN.
