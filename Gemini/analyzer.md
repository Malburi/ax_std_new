...
|--------------|---------|----------|----------|
| `harness-init` Lite | lite/gemini-flash | A | — |
| `harness-init` Standard | init/gemini-flash | A, B(조건부) | D (DB 접속 가능 시) |
| `harness-init` Full | init/gemini-pro | A, B, C | D (DB 접속 가능 시) |
| `analyze-impact` | incremental/gemini-flash | A 캐시 활용 + B Step 8/9/10 | — |
| `safe-modify` | incremental/gemini-flash | A 캐시 활용 + B Step 8/10/11 | — |
| `scaffold-feature` | incremental/gemini-flash | A 캐시 활용 + B Step 8 | — |
| `plan-migration` | init/gemini-pro | A, B 전체, D | — |
| `review-sql` | incremental/gemini-flash | A + B Step 9/10, D | — |
