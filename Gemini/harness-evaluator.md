...
2. gemini.md, skills/, patterns/ 에 각 레이어가 언급되었는지 확인
...
```
샘플링 대상:
1. gemini.md의 "주요 파일 위치" 테이블에서 경로 2개 → Glob으로 실제 존재 확인
...
**목적:** harness를 사용해 향후 작업 시 gemini가 충분한 컨텍스트를 가질 수 있는가.

검증 방법:
1. gemini.md "요청 흐름" 섹션이 실제 코드 경로와 일치하는가 (1개 요청 직접 trace)
2. domain-expert.md에 프로젝트 특화 용어·금지 패턴·주의사항이 포함되었는가
3. gemini.md "작업 시 주의사항" 섹션에 analyzer의 "보완 권장" 항목이 반영되었는가
4. `_workspace/00_spec_report.md` 존재 시 → spec 목표·제약·레거시 주의사항이 gemini.md에 반영되었는가
...
| 컨텍스트 품질 | `writer` | domain-expert.md + gemini.md | "요청 흐름 재작성 + 도메인 키워드 추가 + spec 목표 반영" |
