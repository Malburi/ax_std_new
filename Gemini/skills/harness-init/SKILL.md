---
name: harness-init
description: 프로젝트를 심층 분석해 맞춤형 하네스(gemini.md, 5+ 워크플로우 스킬, 도메인 에이전트, 패턴, 인덱스)를 자동 생성하는 오케스트레이터. "하네스 초기화", "하네스 만들어줘", "하네스 다시 초기화", "harness 다시 만들어줘", "프로젝트 분석해서 설정해줘", "이 프로젝트 gemini 설정해줘", "create harness", "initialize harness", "re-initialize harness", "generate project harness", "하네스 업데이트", "하네스 보완", "스킬만 다시 생성", "에이전트만 다시 생성", "validator만 다시 실행", "패턴 추출해줘", "pattern extract" 요청 시 사용. `.gemini/skills/trace.md` 또는 `.gemini/skills/analyze-impact.md`가 없으면 자동 트리거.
---
...
| 확인 대상 | 의미 |
|----------|------|
| `gemini.md` 존재 + "## 변경 이력" 섹션 | 기존 하네스 있음 |
| `.gemini/skills/trace.md` 존재 | 기존 스킬 있음 |
| `.gemini/skills/analyze-impact.md` 존재 | harness-fin v1 이상 |
| `.gemini/agents/domain-expert.md` | 도메인 에이전트 있음 |
...
| 점수 | Tier | 실행 구성 | 스킵 항목 |
|------|------|---------|---------|
| 0~50 | **Lite** | analyzer(lite/gemini-flash) → writer(gemini-flash) → validator | pattern-extractor, QA 스킵 |
| 51~120 | **Standard** | analyzer(init/gemini-flash, 스택 해당 Phase B만) → writer(gemini-flash) → pattern(병렬) → validator | QA 스킵 |
...
| 기존 + "다시"·"새로" | **재초기화** | `.gemini/backup/[YYYYMMDD-HHMMSS]/`로 백업 후 전체 실행 (analyzer init 모드) |
...
- 백업 대상: `gemini.md`, `.gemini/skills/*.md` (harness-init.md 제외), `.gemini/agents/*.md` (공통 에이전트는 제외, 프로젝트 전용만), `.gemini/patterns/`
- 백업 위치: `.gemini/backup/[YYYYMMDD-HHMMSS]/`
...
| Tier | mode | model |
|------|------|-------|
| Lite | `lite` (Phase A만) | gemini-flash |
| Standard | `init` (A + 스택 해당 Phase B만) | gemini-flash |
| Full | `init` (A + B 전체) | gemini-pro |
...
`.gemini/agents/analyzer.md`의 지침 따름. 완료 후 결과 파일 존재 확인.
...
Agent(
  ...
  ## 3. 패턴 파일 참조
  .gemini/patterns/*.md 각 파일의 용도 및 scaffold-feature와의 연계
  ...
  ## 6. 주의사항
  gemini.md 의 '작업 시 주의사항' 핵심 항목 요약 (3~5개)
...
완료 후 `.gemini/ito-guide.md` 존재 확인. 실패해도 파이프라인 계속 진행 ("ito-guide 미생성" WARN으로 처리).
...
Agent(
  subagent_type="general-purpose",
  description="패턴 추출",
  prompt="<pattern-extractor 에이전트 지침. 프로젝트 루트: [절대경로]. 입력: .gemini/patterns/*.md 스켈레톤 + _workspace/01_analyzer_report.md. 출력: 패턴 파일 본문 + _workspace/05_patterns_extracted.md>",
  model="gemini-flash"
)
```
...
Agent(
  ...
  입력: _workspace/01_analyzer_report.md, _workspace/03_validator_report.md,
        생성된 harness 파일들 (gemini.md, .gemini/skills/, .gemini/agents/, .gemini/patterns/),
...
[Core]
- gemini.md
- .gemini/ito-guide.md               (사용 설명서)
- .gemini/skills/trace.md, scaffolder.md, find-logic.md
...
- .gemini/skills/plan-migration.md          (생성 조건 충족 시)
- .gemini/skills/review-sql.md              (DB 사용 시)
...
다음 단계:
  git add gemini.md .gemini/ && git commit -m "docs: add project harness (harness-fin v1)"
