...
> **ITO/SI 조직을 위한 Gemini Code 확장 하네스**
>
> 처음 투입된 프로젝트에서 _"이 코드 뭐야?"_ 부터 _"이 변경 안전해?"_ 까지,
> Gemini가 프로젝트를 이해하고 작업을 함께 수행할 수 있도록 돕는 에이전트 팀 + 워크플로우 도구 모음입니다.
...
Gemini Code를 아무 설정 없이 사용하면 매번 _"이 프로젝트가 뭔지"_ 처음부터 설명해야 합니다.
harness-ito는 프로젝트를 **한 번 분석해 Gemini가 기억할 수 있는 형태로 저장**합니다.

그 결과, 이후 작업에서 Gemini는 다음을 알고 시작합니다:
...
> 💡 **설치가 안 될 때** — Gemini Code 버전을 확인하세요. 플러그인 기능은 최신 버전에서 지원됩니다.

---

## 🚀 처음 시작하기 (하네스 초기화)

분석하고 싶은 **프로젝트 루트**에서 Gemini Code를 열고 아래 중 하나를 입력하세요:
...
완료되면 프로젝트 루트에 `GEMINI.md`와 `.gemini/` 폴더가 생깁니다.
...
> HOLD / STOP 이 나오면 Gemini가 이유와 권고 조치를 함께 알려줍니다. **자동 수정은 하지 않습니다.**
...
"코드 바꿨는데 문서 동기화해줘"
```
→ `doc-syncer` → GEMINI.md · README · API 문서 업데이트 권고 목록

---

## 🏭 운영 환경 키워드

변경 작업 요청 시 상황을 알려주면 Gemini가 안전성 판단 기준을 맞춰줍니다:
...
```
프로젝트/
├── GEMINI.md                          ← 🧠 이 프로젝트의 핵심 가이드 (git 커밋 권장)
└── .gemini/
...
> 💡 `GEMINI.md`와 `.gemini/`는 팀원과 공유하기 위해 git에 커밋하세요.
...
초기화가 끝나면 `.gemini/ito-guide.md` 파일이 자동으로 생성됩니다.
...
- ✅ **GO** — 진행해도 안전합니다.
- ⚠️ **HOLD** — 주의가 필요한 부분이 있습니다. Gemini가 상세 내용을 알려줍니다. 확인 후 결정하세요.
- 🛑 **STOP** — 현재 방식으로는 진행하지 않는 것을 권고합니다. 대안을 논의하세요.

Gemini는 HOLD/STOP 상황에서도 자동 수정을 하지 않습니다. **판단은 항상 사람이 합니다.**
...
- [Gemini Code 공식 문서](https://docs.anthropic.com/en/docs/gemini-code/overview)
