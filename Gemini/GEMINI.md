...
`.gemini/` 폴더에 *코드베이스 분석 + 수정/개발/마이그레이션 작업까지* 지원하는 에이전트 팀과 워크플로우 스킬이 포함되어 있다.

대상 프로젝트의 코드베이스를 분석해 맞춤형 gemini.md / 워크플로우 스킬 / 도메인 에이전트 / 패턴 / 인덱스를 한 번에 생성하고, 이후 *수정·개발·마이그레이션 작업*까지 끊김 없이 지원한다.
...
| 경로 | 역할 |
|------|------|
| `.gemini-plugin/marketplace.json` | 마켓플레이스 카탈로그 (단일 저장소 = 단일 플러그인) |
| `.gemini-plugin/plugin.json` | 플러그인 매니페스트 |
...
> 본 저장소 내의 `agents/`·`skills/` 경로는 *플러그인 소스*이며, 설치된 대상 프로젝트에서 출력되는 결과물은 여전히 대상 프로젝트의 `.gemini/skills/...`·`.gemini/agents/...`에 기록된다. 에이전트/스킬 본문 내부의 `.gemini/...` 경로는 *대상 프로젝트* 경로를 의미한다.
...
| 2026-06-02 | gemini Code 플러그인 표준 레이아웃으로 재구성 — `.gemini/agents/`·`.gemini/skills/` 중복본 제거, 루트 `agents/`·`skills/<name>/SKILL.md` 단일 source-of-truth로 정리, `.gemini-plugin/marketplace.json`·`plugin.json` 추가 | 저장소 구조 | `/plugin marketplace add Malburi/harness-ito`로 설치 가능하도록 |
...
| 2026-06-03 | 로직 탐색 에이전트 2종 추가 — `logic-tracer`(진입점→DB 처리 흐름 추적) + `feature-finder`(기능명·키워드 코드 위치 탐색) + 트리거 스킬 2종(`trace-logic`, `find-feature`) | agents / skills / gemini.md | 특정 로직·기능 위치 탐색 요구 대응 |
| 2026-06-03 | 에이전트 모델 최적화 — gemini-pro→gemini-flash 8종(change-safety, pattern-extractor, validator, qa, test-generator, sql-reviewer, doc-syncer, logic-tracer). gemini-pro 유지 5종(analyzer, writer, legacy-decoder, impact-analyzer, migration-planner) | agents/ | 패턴 기반 처리 작업에 gemini-pro 불필요, 비용 절감 |
| 2026-06-03 | harness-init 3-Tier 적응 실행 — 복잡도 점수(파일수+DB/ORM+레거시+멀티모듈+외부시스템) 기반 Lite/Standard/Full 자동 분기. Lite: analyzer lite(gemini-flash)+writer(gemini-flash)+validator, QA·pattern 스킵. Standard: analyzer init(gemini-flash)+선택적 Phase B+writer(gemini-flash)+pattern+validator, QA 스킵. Full: 기존 파이프라인. 사용자 override 키워드 지원. | skills/harness-init / agents/analyzer | 프로젝트 규모 무관 전체 파이프라인 실행으로 인한 토큰·시간 낭비 해소 |
| 2026-06-04 | harness-clean 스킬 추가 — gemini.md·.gemini/skills/·agents/·patterns/·_workspace/ 안전 제거 + 플러그인 언인스톨 안내 | skills/harness-clean / gemini.md | 마음에 안 들면 롤백할 수 있어야 표준으로 쓸 수 있음 |
| 2026-06-04 | Ouroboros 명세 게이트 + Karpathy eval 루프 접목 — spec-clarifier(gemini-flash, Phase -1: 소크라테스 인터뷰·모호성점수·GO신호) + harness-evaluator(gemini-flash, Phase 4: 4차원 품질평가·PASS/PARTIAL/RETRY·fix_targets 기반 타겟 재생성·1회 루프) + spec-gate 스킬 + harness-init Phase -1/-4 추가 + analyzer에 spec_context 전달 | agents/spec-clarifier, agents/harness-evaluator, skills/spec-gate, skills/harness-init, gemini.md | 명세 모호성 제거(Ouroboros)·자기 개선 루프(Karpathy)로 harness 생성 품질 향상 |
